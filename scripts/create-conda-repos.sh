#!/usr/bin/env bash
# create-conda-repos.sh — creates three Conda repositories: hosted, proxy, group.
#
# Usage:
#   ./scripts/create-conda-repos.sh
#   BASE_URL=http://192.168.1.10:8080 ./scripts/create-conda-repos.sh
#   BLOB_STORE=s3-primary ./scripts/create-conda-repos.sh
#
...
#   HOSTED_NAME     — hosted repo name                (default: conda-hosted)
#   PROXY_NAME      — proxy repo name                 (default: conda-proxy)
#   GROUP_NAME      — group repo name                 (default: conda-group)
#   PROXY_REMOTE    — remote URL for proxy            (default: https://conda.anaconda.org/conda-forge)
...
HOSTED_NAME="${HOSTED_NAME:-conda-hosted}"
PROXY_NAME="${PROXY_NAME:-conda-proxy}"
GROUP_NAME="${GROUP_NAME:-conda-group}"
PROXY_REMOTE="${PROXY_REMOTE:-https://conda.anaconda.org/conda-forge}"
...
info "Creating Conda HOSTED repository: ${HOSTED_NAME} (blob store: ${BLOB_STORE})"
create_repo conda hosted "$(cat <<JSON
{
  "name": "${HOSTED_NAME}",
  "online": true,
  "allowAnonymous": false,
  "description": "Conda hosted repository",
  "blobStoreId": "${BLOB_STORE_ID}"
}
JSON
)"

info "Creating Conda PROXY repository: ${PROXY_NAME} → ${PROXY_REMOTE} (blob store: ${BLOB_STORE})"
create_repo conda proxy "$(cat <<JSON
{
  "name": "${PROXY_NAME}",
  "online": true,
  "allowAnonymous": ${ALLOW_ANON_PUB},
  "description": "Conda proxy → ${PROXY_REMOTE}",
  "blobStoreId": "${BLOB_STORE_ID}",
  "proxyConfig": {
...
info "Creating Conda GROUP repository: ${GROUP_NAME} (members: ${HOSTED_NAME}, ${PROXY_NAME})"
create_repo conda group "$(cat <<JSON
{
  "name": "${GROUP_NAME}",
  "online": true,
  "allowAnonymous": ${ALLOW_ANON_PUB},
  "description": "Conda group (hosted + proxy)",
  "formatConfig": {
...
echo
info "Example: configure conda to use group channel"
echo "  conda config --add channels ${BASE_URL}/repository/${GROUP_NAME}/"
echo "  conda config --set channel_priority strict"
echo
info "Example: install package via group"
echo "  conda install numpy --override-channels -c ${BASE_URL}/repository/${GROUP_NAME}/"
