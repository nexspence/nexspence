#!/usr/bin/env bash
# create-conan-repos.sh — creates three Conan (C/C++) repositories: hosted, proxy, group.
#
# Usage:
#   ./scripts/create-conan-repos.sh
#   BASE_URL=http://192.168.1.10:8080 ./scripts/create-conan-repos.sh
#   BLOB_STORE=s3-secondary ./scripts/create-conan-repos.sh
#
...
#   HOSTED_NAME     — hosted repo name                (default: conan-hosted)
#   PROXY_NAME      — proxy repo name                 (default: conan-proxy)
#   GROUP_NAME      — group repo name                 (default: conan-group)
#   PROXY_REMOTE    — remote URL for proxy            (default: https://center.conan.io)
...
HOSTED_NAME="${HOSTED_NAME:-conan-hosted}"
PROXY_NAME="${PROXY_NAME:-conan-proxy}"
GROUP_NAME="${GROUP_NAME:-conan-group}"
PROXY_REMOTE="${PROXY_REMOTE:-https://center.conan.io}"
...
info "Creating Conan HOSTED repository: ${HOSTED_NAME} (blob store: ${BLOB_STORE})"
create_repo conan hosted "$(cat <<JSON
{
  "name": "${HOSTED_NAME}",
  "online": true,
  "allowAnonymous": false,
  "description": "Conan (C/C++) hosted repository",
  "blobStoreId": "${BLOB_STORE_ID}"
}
JSON
)"

info "Creating Conan PROXY repository: ${PROXY_NAME} → ${PROXY_REMOTE} (blob store: ${BLOB_STORE})"
create_repo conan proxy "$(cat <<JSON
{
  "name": "${PROXY_NAME}",
  "online": true,
  "allowAnonymous": ${ALLOW_ANON_PUB},
  "description": "Conan proxy → ${PROXY_REMOTE}",
  "blobStoreId": "${BLOB_STORE_ID}",
  "proxyConfig": {
...
info "Creating Conan GROUP repository: ${GROUP_NAME} (members: ${HOSTED_NAME}, ${PROXY_NAME})"
create_repo conan group "$(cat <<JSON
{
  "name": "${GROUP_NAME}",
  "online": true,
  "allowAnonymous": ${ALLOW_ANON_PUB},
  "description": "Conan group (hosted + proxy)",
  "formatConfig": {
...
echo
info "Example: configure conan remote"
echo "  conan remote add nexspence ${BASE_URL}/repository/${GROUP_NAME}/"
echo "  conan remote login nexspence admin"
