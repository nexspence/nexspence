#!/usr/bin/env bash
# create-npm-repos.sh — creates three npm repositories: hosted, proxy, group.
#
# Usage:
#   ./scripts/create-npm-repos.sh
#   BASE_URL=http://192.168.1.10:8080 ./scripts/create-npm-repos.sh
#   BLOB_STORE=s3-primary ./scripts/create-npm-repos.sh
#
...
#   HOSTED_NAME     — hosted repo name                (default: npm-hosted)
#   PROXY_NAME      — proxy repo name                 (default: npm-proxy)
#   GROUP_NAME      — group repo name                 (default: npm-group)
#   PROXY_REMOTE    — remote URL for proxy            (default: https://registry.npmjs.org)
...
HOSTED_NAME="${HOSTED_NAME:-npm-hosted}"
PROXY_NAME="${PROXY_NAME:-npm-proxy}"
GROUP_NAME="${GROUP_NAME:-npm-group}"
PROXY_REMOTE="${PROXY_REMOTE:-https://registry.npmjs.org}"
...
info "Creating npm HOSTED repository: ${HOSTED_NAME} (blob store: ${BLOB_STORE})"
create_repo npm hosted "$(cat <<JSON
{
  "name": "${HOSTED_NAME}",
  "online": true,
  "allowAnonymous": false,
  "description": "npm hosted repository",
  "blobStoreId": "${BLOB_STORE_ID}"
}
JSON
)"

info "Creating npm PROXY repository: ${PROXY_NAME} → ${PROXY_REMOTE} (blob store: ${BLOB_STORE})"
create_repo npm proxy "$(cat <<JSON
{
  "name": "${PROXY_NAME}",
  "online": true,
  "allowAnonymous": ${ALLOW_ANON_PUB},
  "description": "npm proxy → ${PROXY_REMOTE}",
  "blobStoreId": "${BLOB_STORE_ID}",
  "proxyConfig": {
...
info "Creating npm GROUP repository: ${GROUP_NAME} (members: ${HOSTED_NAME}, ${PROXY_NAME})"
create_repo npm group "$(cat <<JSON
{
  "name": "${GROUP_NAME}",
  "online": true,
  "allowAnonymous": ${ALLOW_ANON_PUB},
  "description": "npm group (hosted + proxy)",
  "formatConfig": {
...
echo
info "Example: configure npm to use group registry"
echo "  npm config set registry ${BASE_URL}/repository/${GROUP_NAME}/"
echo
info "Example: publish to hosted"
echo "  npm publish --registry ${BASE_URL}/repository/${HOSTED_NAME}/"
