#!/usr/bin/env bash
# create-cargo-repos.sh — creates three Cargo (Rust) repositories: hosted, proxy, group.
#
# Usage:
#   ./scripts/create-cargo-repos.sh
#   BASE_URL=http://192.168.1.10:8080 ./scripts/create-cargo-repos.sh
#   BLOB_STORE=s3-primary ./scripts/create-cargo-repos.sh
#
...
#   HOSTED_NAME     — hosted repo name                (default: cargo-hosted)
#   PROXY_NAME      — proxy repo name                 (default: cargo-proxy)
#   GROUP_NAME      — group repo name                 (default: cargo-group)
#   PROXY_REMOTE    — remote URL for proxy            (default: https://index.crates.io)
...
HOSTED_NAME="${HOSTED_NAME:-cargo-hosted}"
PROXY_NAME="${PROXY_NAME:-cargo-proxy}"
GROUP_NAME="${GROUP_NAME:-cargo-group}"
PROXY_REMOTE="${PROXY_REMOTE:-https://index.crates.io}"
...
info "Creating Cargo HOSTED repository: ${HOSTED_NAME} (blob store: ${BLOB_STORE})"
create_repo cargo hosted "$(cat <<JSON
{
  "name": "${HOSTED_NAME}",
  "online": true,
  "allowAnonymous": false,
  "description": "Cargo (Rust) hosted repository",
  "blobStoreId": "${BLOB_STORE_ID}"
}
JSON
)"

info "Creating Cargo PROXY repository: ${PROXY_NAME} → ${PROXY_REMOTE} (blob store: ${BLOB_STORE})"
create_repo cargo proxy "$(cat <<JSON
{
  "name": "${PROXY_NAME}",
  "online": true,
  "allowAnonymous": ${ALLOW_ANON_PUB},
  "description": "Cargo proxy → ${PROXY_REMOTE}",
  "blobStoreId": "${BLOB_STORE_ID}",
  "proxyConfig": {
...
info "Creating Cargo GROUP repository: ${GROUP_NAME} (members: ${HOSTED_NAME}, ${PROXY_NAME})"
create_repo cargo group "$(cat <<JSON
{
  "name": "${GROUP_NAME}",
  "online": true,
  "allowAnonymous": ${ALLOW_ANON_PUB},
  "description": "Cargo group (hosted + proxy)",
  "formatConfig": {
...
echo
info "Example: configure Cargo to use group (sparse index)"
echo "  # ~/.cargo/config.toml"
echo "  [source.nexspence]"
echo "  registry = \"${BASE_URL}/repository/${GROUP_NAME}/\""
echo "  [source.crates-io]"
echo "  replace-with = \"nexspence\""
