#!/usr/bin/env bash
# create-nuget-repos.sh — creates three NuGet repositories: hosted, proxy, group.
#
# Usage:
#   ./scripts/create-nuget-repos.sh
#   BASE_URL=http://192.168.1.10:8080 ./scripts/create-nuget-repos.sh
#   BLOB_STORE=s3-secondary ./scripts/create-nuget-repos.sh
#
...
#   HOSTED_NAME     — hosted repo name                (default: nuget-hosted)
#   PROXY_NAME      — proxy repo name                 (default: nuget-proxy)
#   GROUP_NAME      — group repo name                 (default: nuget-group)
#   PROXY_REMOTE    — remote URL for proxy            (default: https://api.nuget.org/v3/index.json)
...
HOSTED_NAME="${HOSTED_NAME:-nuget-hosted}"
PROXY_NAME="${PROXY_NAME:-nuget-proxy}"
GROUP_NAME="${GROUP_NAME:-nuget-group}"
PROXY_REMOTE="${PROXY_REMOTE:-https://api.nuget.org/v3/index.json}"
...
info "Creating NuGet HOSTED repository: ${HOSTED_NAME} (blob store: ${BLOB_STORE})"
create_repo nuget hosted "$(cat <<JSON
{
  "name": "${HOSTED_NAME}",
  "online": true,
  "allowAnonymous": false,
  "description": "NuGet hosted repository",
  "blobStoreId": "${BLOB_STORE_ID}"
}
JSON
)"

info "Creating NuGet PROXY repository: ${PROXY_NAME} → ${PROXY_REMOTE} (blob store: ${BLOB_STORE})"
create_repo nuget proxy "$(cat <<JSON
{
  "name": "${PROXY_NAME}",
  "online": true,
  "allowAnonymous": ${ALLOW_ANON_PUB},
  "description": "NuGet proxy → ${PROXY_REMOTE}",
  "blobStoreId": "${BLOB_STORE_ID}",
  "proxyConfig": {
...
info "Creating NuGet GROUP repository: ${GROUP_NAME} (members: ${HOSTED_NAME}, ${PROXY_NAME})"
create_repo nuget group "$(cat <<JSON
{
  "name": "${GROUP_NAME}",
  "online": true,
  "allowAnonymous": ${ALLOW_ANON_PUB},
  "description": "NuGet group (hosted + proxy)",
  "formatConfig": {
...
echo "  hosted : ${BASE_URL}/repository/${HOSTED_NAME}/index.json"
echo "  proxy  : ${BASE_URL}/repository/${PROXY_NAME}/index.json"
echo "  group  : ${BASE_URL}/repository/${GROUP_NAME}/index.json"
echo
info "Example: add group source (dotnet CLI)"
echo "  dotnet nuget add source ${BASE_URL}/repository/${GROUP_NAME}/index.json --name nexspence"
echo
info "Example: push package to hosted"
echo "  dotnet nuget push MyPkg.1.0.0.nupkg --source ${BASE_URL}/repository/${HOSTED_NAME}/index.json --api-key admin123"
