#!/usr/bin/env bash
# create-maven-repos.sh — creates three Maven 2/3 repositories: hosted, proxy, group.
#
# Usage:
#   ./scripts/create-maven-repos.sh
#   BASE_URL=http://192.168.1.10:8080 HOSTED_NAME=my-maven ./scripts/create-maven-repos.sh
#   BLOB_STORE=s3-primary ./scripts/create-maven-repos.sh
#
...
#   HOSTED_NAME     — hosted repo name                (default: maven-hosted)
#   PROXY_NAME      — proxy repo name                 (default: maven-proxy)
#   GROUP_NAME      — group repo name                 (default: maven-group)
#   PROXY_REMOTE    — remote URL for proxy            (default: https://repo1.maven.org/maven2)
...
HOSTED_NAME="${HOSTED_NAME:-maven-hosted}"
PROXY_NAME="${PROXY_NAME:-maven-proxy}"
GROUP_NAME="${GROUP_NAME:-maven-group}"
PROXY_REMOTE="${PROXY_REMOTE:-https://repo1.maven.org/maven2}"
...
info "Creating Maven2 HOSTED repository: ${HOSTED_NAME} (blob store: ${BLOB_STORE})"
create_repo maven2 hosted "$(cat <<JSON
{
  "name": "${HOSTED_NAME}",
  "online": true,
  "allowAnonymous": false,
  "description": "Maven2 hosted repository",
  "blobStoreId": "${BLOB_STORE_ID}"
}
JSON
)"

info "Creating Maven2 PROXY repository: ${PROXY_NAME} → ${PROXY_REMOTE} (blob store: ${BLOB_STORE})"
create_repo maven2 proxy "$(cat <<JSON
{
  "name": "${PROXY_NAME}",
  "online": true,
  "allowAnonymous": ${ALLOW_ANON_PUB},
  "description": "Maven2 proxy → ${PROXY_REMOTE}",
  "blobStoreId": "${BLOB_STORE_ID}",
  "proxyConfig": {
...
info "Creating Maven2 GROUP repository: ${GROUP_NAME} (members: ${HOSTED_NAME}, ${PROXY_NAME})"
create_repo maven2 group "$(cat <<JSON
{
  "name": "${GROUP_NAME}",
  "online": true,
  "allowAnonymous": ${ALLOW_ANON_PUB},
  "description": "Maven2 group (hosted + proxy)",
  "formatConfig": {
...
echo
info "Example: publish artifact to hosted (Maven settings.xml)"
echo "  <repository><id>nexspence</id><url>${BASE_URL}/repository/${HOSTED_NAME}/</url></repository>"
echo
info "Example: resolve via group (Maven settings.xml mirror)"
echo "  <mirror><url>${BASE_URL}/repository/${GROUP_NAME}/</url><mirrorOf>*</mirrorOf></mirror>"
