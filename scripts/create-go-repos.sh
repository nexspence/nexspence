#!/usr/bin/env bash
# create-go-repos.sh — creates three Go (GOPROXY) repositories: hosted, proxy, group.
#
# Usage:
#   ./scripts/create-go-repos.sh
#   BASE_URL=http://192.168.1.10:8081 HOSTED_NAME=my-go ./scripts/create-go-repos.sh
#   BLOB_STORE=s3-prod ./scripts/create-go-repos.sh     # put everything in the "s3-prod" store
#
...
#   HOSTED_NAME     — hosted repo name                (default: go-hosted)
#   PROXY_NAME      — proxy repo name                 (default: go-proxy)
#   GROUP_NAME      — group repo name                 (default: go-group)
#   PROXY_REMOTE    — remote URL for proxy            (default: https://proxy.golang.org)
...
HOSTED_NAME="${HOSTED_NAME:-go-hosted}"
PROXY_NAME="${PROXY_NAME:-go-proxy}"
GROUP_NAME="${GROUP_NAME:-go-group}"
PROXY_REMOTE="${PROXY_REMOTE:-https://proxy.golang.org}"
...
# ── 1. Hosted ─────────────────────────────────────────────────────────────────────
info "Creating Go HOSTED repository: ${HOSTED_NAME} (blob store: ${BLOB_STORE})"
create_repo go hosted "$(cat <<JSON
{
  "name": "${HOSTED_NAME}",
  "online": true,
  "allowAnonymous": false,
  "description": "Go hosted repository",
...
# ── 2. Proxy ──────────────────────────────────────────────────────────────────────
info "Creating Go PROXY repository: ${PROXY_NAME} → ${PROXY_REMOTE} (blob store: ${BLOB_STORE})"
create_repo go proxy "$(cat <<JSON
{
  "name": "${PROXY_NAME}",
  "online": true,
  "allowAnonymous": ${ALLOW_ANON_PUB},
  "description": "Go proxy → ${PROXY_REMOTE}",
  "blobStoreId": "${BLOB_STORE_ID}",
  "proxyConfig": {
...
# ── 3. Group ──────────────────────────────────────────────────────────────────────
info "Creating Go GROUP repository: ${GROUP_NAME} (members: ${HOSTED_NAME}, ${PROXY_NAME})"
create_repo go group "$(cat <<JSON
{
  "name": "${GROUP_NAME}",
  "online": true,
  "allowAnonymous": ${ALLOW_ANON_PUB},
  "description": "Go group (hosted + proxy)",
  "formatConfig": {
...
echo
info "Example: use proxy directly"
echo "  GOPROXY=${BASE_URL}/repository/${PROXY_NAME} go get github.com/gin-gonic/gin@v1.9.1"
echo
info "Example: use group (proxy + hosted fallback)"
echo "  GOPROXY=${BASE_URL}/repository/${GROUP_NAME} go get github.com/rs/zerolog@v1.32.0"
echo
info "Example: use group for all modules (set in go env)"
echo "  go env -w GOPROXY=${BASE_URL}/repository/${GROUP_NAME},direct"
echo
info "Example: check available versions"
echo "  GOPROXY=${BASE_URL}/repository/${GROUP_NAME} go list -m -versions github.com/jackc/pgx/v5"
echo
